...
def sign_up(email: str, display_name: str, password: str) -> tuple[bool, str]:
    ...
    try:
        ...
        user = res.user
        if not user: return False, "Sign-up failed."
...
def _ensure_profile_exists(user_id: str, email: str, display_name: str | None = None, eager: bool = False) -> None:
    ...
    if not exists:
        payload = {"id": user_id, "email": email, "display_name": display_name or email.split("@")[0], "role": "user", "status": "pending"}
...
def try_bootstrap_current_user_as_primary_admin(code: str) -> tuple[bool, str]:
    if not st.session_state.get("auth_user"): return False, "You must sign in first."
    if not needs_admin_bootstrap(): return False, "A Primary Admin already exists."
    expected = st.secrets.get("PRIMARY_ADMIN_BOOTSTRAP_CODE", "")
    if not expected or code.strip() != str(expected).strip(): return False, "Invalid bootstrap code."
    sb = get_supabase()
