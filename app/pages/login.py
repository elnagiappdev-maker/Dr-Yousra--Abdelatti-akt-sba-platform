    ...
    if st.session_state.get("auth_user"):
        ...
        with tab_signup:
            ...
            if st.button("Create Account"):
                if not email2 or not pwd2: st.error("Email and password are required.")
                elif pwd2 != pwdc: st.error("Passwords do not match.")
                else:
                    ok, msg = sign_up(email2, name2 or email2.split('@')[0], pwd2)
                    st.success(msg) if ok else st.error(msg)
                    if ok: st.info("Please sign in now. Your status is **pending** until an admin activates you.")
